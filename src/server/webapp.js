...
const doPost = async(e) => {
        try {
            // TO DO SOMETHINGS...
            const fmSheet = ['TRUE', 'AIS', 'TOT', 'FINET', '3BB'];
            const fmStatus = ['OPEN', 'IN PROGRESS', 'CLOSE'];
            const fmCommandRegex = new RegExp(
                `#(${fmSheet.join('|')})\\s*([\\d]{6})\\s*(\\b${fmStatus.join('|')}\\b)`,
                'i'
            );
            ...
            if (fmCommandRegex.test(messages)) {
                ...
                if (record) {
                    //ใช้  regex ทำให้ตัวอักษรตัวแรก หรือหลังจาก . หรือหลังจาก เว้นวรรค เป็นตัวพิมพ์ใหญ่
                    record['หมายเหตุ'] = _command.replace(/^(.)|\s+(.)/g, (c) => c.toUpperCase());
                    record.save();
                    await replyMessage(
